...
Usage inside an except clause:
    ...
    try:
        ...
        log.error(report.for_claude())       # heavy / LLM-friendly edition
...
This file is a copy of the standalone error_handler.py at the project root.
We vendor it inside the package so the app can ship as a single import
without sys.path acrobatics. Keep them in sync if either is edited.
"""
...
class ErrorReport:
    ...
    Three output flavors:
      ...
      for_claude()   heavy / LLM-friendly edition
    """
...
# ---------------------------------------------------------------------------
# Chain walker with cycle and depth guards
# ---------------------------------------------------------------------------
...
def describe_error(exc=None, *, include_locals=False, max_chain_depth=10):
    ...
    Args:
        ...
        include_locals: capture frame.f_locals (off by default - frame locals
            can contain secrets that shouldn't leak into logs)
...
# ---------------------------------------------------------------------------
# Concise formatter (Python-style traceback with extras)
# ---------------------------------------------------------------------------
...
__all__ = ["describe_error", "ErrorReport"]
